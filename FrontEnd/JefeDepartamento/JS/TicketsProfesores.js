...
async function cargarTickets() {
    try {
        const res = await fetch("http://localhost:5067/api/tickets/admin/profesores");
        if(res.ok) {
...
function renderTabla(lista) {
    ...
    if(lista.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;">No hay tickets de docentes.</td></tr>`;
        return;
    }
...
window.cambiarEstado = async function(nuevoEstado) {
    ...
    try {
        const res = await fetch("http://localhost:5067/api/tickets/admin/estatus", {
            ...
            body: JSON.stringify({ IdTicket: ticketSeleccionado.idTicket, Estatus: estatusId, Tipo: "profesor" })
        });
